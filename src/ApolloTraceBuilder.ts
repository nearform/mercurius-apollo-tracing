// This class is a helper for mercuriusApolloTracing
import {
  DocumentNode,
  GraphQLError,
  GraphQLResolveInfo,
...
/**
 * The code is a port of https://github.com/apollographql/apollo-server/blob/bb86f59b7b4ebbcf8776c453e3399ad19796405b/packages/apollo-server-core/src/plugin/traceTreeBuilder.ts for mercurius
 */
export class ApolloTraceBuilder {
  ...
  public stopTiming() {
    ...
    this.trace.durationNs = durationHrTimeToNanoseconds(
  ...
  public willResolveField(info: GraphQLResolveInfo): () => void {
    ...
    node.startTime = durationHrTimeToNanoseconds(
      process.hrtime(this.startHrTime)
    )
    ...
    return () => {
      node.endTime = durationHrTimeToNanoseconds(
        process.hrtime(this.startHrTime)
      )
    }
  }
